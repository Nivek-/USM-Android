...
public class CargandoActivity extends Activity {
	...
	int progreso;// Guardará un numero entero que será el numero de segundos que
					// ha pasado desde que hemos inicializado la aplicación.
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		barraProgreso = (ProgressBar) findViewById(R.id.progressBar1);// Relacionamos
																		// el la
																		// barra
																		// de
																		// progreso
																		...
																		// el de
																		// java
		new TareaSegundoPlano().execute();// Iniciamos la tarea en segundo
											// plano, en este caso no
											// necesitamos pasarle nada.
	}
	...
	public class TareaSegundoPlano extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			barraProgreso.setMax(10);// El máximo de la barra de progreso son
										// 10, de los 10 segundo que va a durar
		...
		protected Void doInBackground(Void... params) {
			for (int progreso = 1; progreso <= 10; progreso++) {// Creamos un
																// for de 1 a 10
																// que irá
																// contando los
																// segundos.
				...
				try {
					Thread.sleep(1000);// Esto lo que hace es ralentizar este
										// proceso un segundo (el tiempo que se
										// pone entre paréntesis es en
										// milisegundos) tiene que ir entre try
										// y catch
				...
				publishProgress();// Actualizamos el progreso, es decir al
									// llamar a este proceso en realidad estamos
									// llamamos al método onProgresssUpdate()
			}

			return null;// Al llegar aquí, no devolvemos nada y acaba la tarea
		...
		protected void onPostExecute(Void result) {// A este método se le llama
			...
			//Intent intent = new Intent("android.intent.action.ProfesorActivity");
			//startActivity(intent);
			//finish();

			if (variables_publicas.getTipoUser() == "Profesor") {
				 Intent intentP = new
				 Intent("android.intent.action.Menu_profesor");
				 startActivity(intentP);
				 finish();
			} else if (variables_publicas.getTipoUser() == "Alumno") {
				 Intent intentA = new
				 Intent("android.intent.action.Menu_profesor");
				 startActivity(intentA);
				 finish();

			}else if (variables_publicas.getTipoUser()== "Casino"){
				 Intent intentC = new
				 Intent("android.intent.action.CasinoActivity");
